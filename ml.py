...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 23 11:05:57 2021
...
data_chickweight=pd.read_csv('ChickWeight.csv')

#create deep copy of orginal data
data=data_chickweight.copy(deep=True)
...
( ggplot(data) + aes(x='Time',y='weight',color='Diet') +geom_point() )
( ggplot(data) + aes(x='Time',y='weight',color='Diet') +geom_line() )
...
z=np.array([11]).reshape((-1,1))

time_pred=model.predict(z)
print(time_pred)
#############################################################
